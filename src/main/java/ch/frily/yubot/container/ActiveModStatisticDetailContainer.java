...
public class ActiveModStatisticDetailContainer extends Container {
    ...
    public ActiveModStatisticDetailContainer(Member member, Map<Member, List<ActiveModTracking>> activeModTrackingMap) {
        ...
        addFormatedText("**Totale aktive Zeit:** %s", Util.calcDuration(activeModTrackings.stream().mapToInt(ActiveModTracking::activeTime).sum()));
        ...
        for (ActiveModTracking activeModTracking : activeModTrackings) {
            ...
            monthDetail.append(String.format("Beitrag: %.0f%%", calcPercentage(activeModTracking.activeTime(), thisMonthsTotalActiveTime)));
    ...
    private double calcPercentage(int value1, int value2) {
        if (value2 == 0)
            return 0;
